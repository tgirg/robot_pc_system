...
class TestFieldWidget(QWidget):
    ...
    def __init__(self, field_width_mm: float = 4500.0, field_height_mm: float = 2400.0) -> None:
        super().__init__()
        self.field_model = FieldModel.load()
    ...
    def _object_box(self) -> QGroupBox:
        ...
        apply_button = QPushButton("座標反映")
        apply_button.clicked.connect(self.apply_selected_object_position)
        coord_grid.addWidget(apply_button, 0, 2, 2, 1)
    ...
    def apply_selected_object_position(self) -> None:
        item = self._selected_object()
        if item is None:
            self.warning_label.setText("座標を反映するオブジェクトを選択してください。")
            return
        self.move_object_to(item.name, float(self.object_x_spin.value()), float(self.object_y_spin.value()))
