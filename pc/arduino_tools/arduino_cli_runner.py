...
ARDUINO_CLI_FALLBACK = Path(r"C:\Program Files\Arduino CLI\arduino-cli.exe")
...
def _cli_or_error() -> str:
    ...
    if not cli:
        raise FileNotFoundError(
            r"arduino-cli が見つかりません。PATH または C:\Program Files\Arduino CLI\arduino-cli.exe を確認してください。"
        )
    return cli
...
def compile_sketch(sketch_name: str, fqbn: str) -> ArduinoCliResult:
    ...
    return _run_command([_cli_or_error(), "compile", "--fqbn", fqbn, "--build-path", str(cache), sketch_path(sketch_name)], timeout=240)
