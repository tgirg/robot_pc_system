...
class SerialConnection:
    def __init__(self, timeout: float = 0.05, reconnect_interval_s: float = 2.0) -> None:
    ...
    def connect(self, port: str, baudrate: int) -> ConnectionResult:
        ...
        try:
            self.conn = serial.Serial(self.port, self.baudrate, timeout=self.timeout, write_timeout=self.timeout)
            self.last_error = ""
