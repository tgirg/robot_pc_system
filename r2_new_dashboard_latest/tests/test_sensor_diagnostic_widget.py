...
from widgets.sensor_diagnostic_widget import SensorDiagnosticWidget  # noqa: E402
...
def test_widget_renders_explicit_not_configured_state(
    ...
    assert "inventory=DEFINED_PENDING_CHECK" in widget.inventory_state_label.text()
    assert "drive-role nodes excluded=1" in widget.boundary_label.text()
    assert "*_STATUS,DUMMY" in widget.boundary_label.text()
    assert widget.sensor_table.rowCount() == 12
    assert widget.sensor_table.item(0, 0).text() == "FL"
    assert widget.sensor_table.item(0, 1).text() == "GPIO13"
    assert widget.sensor_table.item(7, 0).text() == "US1"
    assert widget.sensor_table.item(7, 1).text() == "GPIO35"
    assert "入力専用" in widget.sensor_table.item(7, 5).text()
    assert widget.unmapped_node_table.rowCount() == 0
    assert "connection-check targets" in widget.empty_state_label.text()
...
def test_widget_keeps_unbound_robot_empty(
    ...
    assert widget.sensor_table.rowCount() == 0
    assert widget.unmapped_node_table.rowCount() == 0


def test_widget_labels_sensor_node_unmapped_and_preserves_global_fault(
    qt_app: QApplication,
    tmp_path: Path,
) -> None:
    robot = _snapshot(tmp_path)
    node = NodeSnapshot(
        "candidate_sensor_node",
        "sensor",
        False,
        NodeDisplayState.PRESENT,
        ("virtual://sensor",),
    )
    robot = replace(robot, nodes=robot.nodes + (node,), fault="sensor safety boundary fault", ready=False)
    fleet = build_fleet_dashboard_snapshot(RobotId.R2, (robot,), now_ms=robot.timestamp_ms)
    widget = SensorDiagnosticWidget()
    widget.set_fleet_snapshot(fleet)
    qt_app.processEvents()
    ...
    assert widget.sensor_table.rowCount() == 12
    assert widget.sensor_table.item(0, 0).text() == "FL"
    assert widget.unmapped_node_table.rowCount() == 1
    assert widget.unmapped_node_table.item(0, 0).text() == "candidate_sensor_node"
    assert widget.unmapped_node_table.item(0, 4).text() == "UNMAPPED"
    assert "FAULT | sensor safety boundary fault" in widget.fault_banner.text()
