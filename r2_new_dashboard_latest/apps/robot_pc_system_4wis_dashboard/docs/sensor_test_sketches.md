...
| `lsb_sensor_node` | LSB基板/ユニバーサル基板の通信確認 | `LSB,ID`, `LSB,I2C`, `LSB,SENS` |
...
`lsb_sensor_node` は、LSB基板の実物がない段階でもユニバーサル基板でアプリ側の通信経路を確認するためのスケッチです。

シリアルモニタで次を確認します。

```text
BOOT,LSB_SENSOR_NODE_READY
FW,lsb_sensor_node,0.1.0
LSB,ID,2026F3F_LSB_REVA_VER0,0.1.0
LSB,I2C,...
LSB,SENS,0,tof=0/0/0/0,us=800/820/900/920/1500/1480/1200/1180,imu=0.0/0.0/0.0
```

使えるコマンドは `PING`、`ID?`、`I2C?`、`SENS?`、`STREAM ON`、`STREAM OFF` です。

現時点の `lsb_sensor_node` は安全な通信確認用です。ToFとIMUの実センサ読み取りは未実装で、LSB実基板のコネクタ向き、レギュレータ、外部5V経路、Gerber/製造状態は確認しません。
