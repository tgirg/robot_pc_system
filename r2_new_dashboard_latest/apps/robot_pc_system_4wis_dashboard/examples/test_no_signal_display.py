...
def main() -> int:
    ...
    if "LSB" not in tab_names:
        raise SystemExit(f"失敗: トップレベルLSBタブが見つかりません: {tab_names}")
    if tab_names.index("LSB") <= tab_names.index("4WIS"):
        raise SystemExit(f"失敗: LSBタブが4WISの次に配置されていません: {tab_names}")
    if not hasattr(window, "lsb_sensor_board_widget"):
        raise SystemExit("失敗: LSB専用ウィジェットが初期化されていません")
