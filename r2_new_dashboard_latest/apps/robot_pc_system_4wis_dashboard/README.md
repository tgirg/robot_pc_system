...
現行`pc_controller`と同じNDJSON Fake経路をMain Dashboard / Drive Diagnostic /
Mechanism Diagnostic / Sensor Diagnostic / Calibrationで共有状態を表示する場合は、
リポジトリルートからrobot割当を明示して起動します。

```cmd
run-fake-robot-dashboard.cmd --robot R2
```

`R1`または`R2`の指定は必須です。この起動方法では旧Serial、COM探索、カメラ、ARM、走行、
calibration出力は作られず、選ばなかったrobotはUNBOUND/OFFLINE表示になります。Drive Diagnosticは
同じshared snapshotから4輪command/observed値、drive node、推進・旋回vectorを表示するだけで送信機能はありません。
Mechanism Diagnosticも同じsnapshotだけを使用し、現時点ではauthoritativeなnon-drive機構定義がないため
`NOT_CONFIGURED`を表示します。将来候補や旧GUIのアクチュエータ値をlive stateとして補完しません。
Sensor Diagnosticも同じsnapshotだけを使用します。現時点ではauthoritativeなper-robot sensor inventory/
telemetry mappingがないため`NOT_CONFIGURED`を表示し、旧Mock/DUMMY値やprototype LSB CSV値をlive stateへ
流用しません。sensor-role nodeが見えても、個別sensor定義がなければ`UNMAPPED`のままです。
Calibrationはcontroller-loaded servo設定を監査し、Servo Zeroの`center_us`/`trim_deg`候補を
GUIメモリ内のlocal pendingとしてvalidate/revertできます。configへのsave、Serial、servo出力、applyは
Controller-level Safety API不在のため常に無効です。
