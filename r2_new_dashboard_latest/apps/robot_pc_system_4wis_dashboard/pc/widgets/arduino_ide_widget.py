...
SKETCH_CHOICES: list[tuple[str, str]] = [
    ("esp32_firmware", "4WIS real firmware"),
    ("drive_controller", "通常制御（安全ダミー）"),
    ("custom_board_fl_gpio13_value_test", "FL GPIO13 値読み取りテスト"),
...
def sketch_directory(sketch_name: str) -> Path:
    ...
    safe_name = safe_name or DEFAULT_SKETCH
...
class ArduinoSerialMonitorPanel(QWidget):
    ...
    def connect_monitor(self) -> None:
        ...
        if self._other_monitor_uses_port(port):
            ...
            return
        host_serial = getattr(self.host, "serial", None)
        status = host_serial.status() if host_serial is not None else None
        if status is not None and status.connected and not status.mock and str(host_serial.port).upper() == port.upper():
            QMessageBox.warning(
                self,
                "COMポート使用中",
...
class ArduinoIdeWidget(QWidget):
    def __init__(self, host) -> None:
        ...
        self.easy_fl_button = make_button(
            "FL GPIO13値読み取りを書き込む",
            lambda: self.easy_upload_sketch("custom_board_fl_gpio13_value_test"),
            54,
        )
        ...
        self.cleanup_serial_button = make_button("他のシリアル通信を停止", self.cleanup_serial_users, 42)
        self.easy_4wis_button = make_button("4WISファームを書き込む", lambda: self.easy_upload_sketch("esp32_firmware"), 54)
    ...
    def _build(self) -> None:
        ...
        self.write_tabs = QTabWidget()
        self.write_tabs.setObjectName("arduinoWriteModeTabs")
        self.write_tabs.setUsesScrollButtons(True)
        layout.addWidget(self.write_tabs, 1)

        easy_page = QWidget()
        easy_layout = QVBoxLayout(easy_page)
        easy_layout.setContentsMargins(0, 0, 0, 0)
        easy_layout.setSpacing(8)
        easy_layout.addWidget(self._easy_write_box())
        easy_layout.addStretch(1)
        self.write_tabs.addTab(easy_page, "かんたん")
        ...
        toolbar_layout.addWidget(self.cleanup_serial_button, 0, 7)
        ...
        detail_page_layout.setSpacing(8)
        detail_page_layout.addWidget(toolbar)
        detail_page_layout.addWidget(detail_group)
        detail_page_layout.addWidget(editor_panel, 1)
        self.write_tabs.addTab(detail_page, "詳細")
        ...
        log_page_layout.setSpacing(8)
        log_page_layout.addWidget(output_buttons)
        log_page_layout.addWidget(self.output_log, 1)
        self.write_tabs.addTab(log_page, "ログ")
    ...
    def _easy_write_box(self) -> QWidget:
        ...
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(8)
        layout.addWidget(self.easy_status_label)

        tab_widget = QTabWidget()
        tab_widget.setObjectName("easyWriteSketchTabs")
        tab_widget.setUsesScrollButtons(True)
        layout.addWidget(tab_widget)

        fl_page = QWidget()
        fl_layout = QGridLayout(fl_page)
        fl_layout.setContentsMargins(0, 0, 0, 0)
        fl_layout.setSpacing(8)
        fl_layout.addWidget(self.easy_find_button, 0, 0)
        fl_layout.addWidget(self.easy_fl_button, 0, 1, 1, 2)
        fl_layout.setColumnStretch(2, 1)
        tab_widget.addTab(fl_page, "FL/GPIO13")

        sensor_page = QWidget()
        sensor_layout = QGridLayout(sensor_page)
        sensor_layout.setContentsMargins(0, 0, 0, 0)
        sensor_layout.setSpacing(8)
        sensor_layout.addWidget(self.easy_imu_button, 0, 0)
        sensor_layout.addWidget(self.easy_optical_button, 0, 1)
        sensor_layout.addWidget(self.easy_lidar_button, 0, 2)
        sensor_layout.addWidget(self.easy_field_demo_button, 1, 0)
        sensor_layout.addWidget(self.easy_optical_example_button, 1, 1)
        sensor_layout.addWidget(self.easy_neopixel_button, 1, 2)
        sensor_layout.setColumnStretch(2, 1)
        tab_widget.addTab(sensor_page, "センサ")

        control_page = QWidget()
        control_layout = QGridLayout(control_page)
        control_layout.setContentsMargins(0, 0, 0, 0)
        control_layout.setSpacing(8)
        control_layout.addWidget(self.easy_drive_button, 0, 0)
        control_layout.addWidget(self.easy_4wis_button, 0, 1)
        control_layout.addWidget(self.easy_selected_button, 0, 2)
        control_layout.setColumnStretch(2, 1)
        tab_widget.addTab(control_page, "制御/その他")
    ...
    def _host_serial_port(self) -> str:
        serial = getattr(self.host, "serial", None)
        if serial is not None:
            return normalize_port_name(str(getattr(serial, "port", "") or ""))
        controller = None
        if hasattr(self.host, "_shared_runtime_controller"):
            controller = self.host._shared_runtime_controller()
        runtime_serial = getattr(controller, "serial", None) if controller is not None else None
        return normalize_port_name(str(getattr(runtime_serial, "port", "") or ""))

    def _host_serial_status(self):
        serial = getattr(self.host, "serial", None)
        if serial is not None and hasattr(serial, "status"):
        ...
        if hasattr(self.host, "_shared_runtime_controller"):
            controller = self.host._shared_runtime_controller()
        runtime_serial = getattr(controller, "serial", None) if controller is not None else None
        if runtime_serial is not None:
            Status.connected = True
            Status.mock = False
        return Status()

    def _disconnect_host_serial_for_upload(self, port: str) -> bool:
        serial = getattr(self.host, "serial", None)
        if serial is not None and hasattr(self.host, "disconnect_esp32_from_ui"):
        ...
        if hasattr(self.host, "_shared_runtime_controller"):
            controller = self.host._shared_runtime_controller()
        runtime_serial = getattr(controller, "serial", None) if controller is not None else None
        if runtime_serial is None:
            return False
        current_port = normalize_port_name(str(getattr(runtime_serial, "port", "") or ""))
        if current_port and current_port.upper() != normalize_port_name(port).upper():
            return False
        try:
            runtime_serial.close()
        except Exception:
            pass
        controller.serial = None
        if hasattr(controller, "args"):
            controller.args.port = port
        return True
    ...
    def refresh_ports(self, log: bool = True) -> None:
        ...
        if preferred and not ports and self.port_combo.findData(preferred) < 0:
    ...
    def select_preferred_port(self, combo: QComboBox, current: str = "") -> None:
        ...
        if preferred and not ports and combo.findData(preferred) < 0:
    ...
    def start_upload_after_confirmation(self) -> None:
        ...
        if answer != QMessageBox.StandardButton.Yes:
            ...
            return
        status = self._host_serial_status()
        if status.connected and not status.mock and self._host_serial_port().upper() == port.upper():
            self._disconnect_host_serial_for_upload(port)
    ...
    def focus_serial_monitor(self) -> None:
        ...
        if monitor is not None and hasattr(monitor, "set_port_and_baud"):
            ...
            return
        self.host.switch_to_tab("センサチェック")
        self.append_output("シリアルタブがないため、センサチェックへ移動しました。")
    ...
    def prepare_serial_monitor_after_upload(self) -> None:
        ...
        if monitor is not None and hasattr(monitor, "set_port_and_baud"):
            ...
            if hasattr(monitor, "prepare_for_immediate_start"):
                QTimer.singleShot(0, lambda: monitor.prepare_for_immediate_start(auto_connect=True))
            return
        self.prepare_sensor_check_after_upload()

    def prepare_sensor_check_after_upload(self) -> None:
        port = self.last_upload_port or self.current_port()
        widget = getattr(self.host, "sensor_check_widget", None)
        if widget is not None and hasattr(widget, "set_ports"):
            widget.set_ports([port], selected=port)
        if hasattr(self.host, "switch_to_tab"):
            self.host.switch_to_tab("センサチェック")
        if hasattr(self.host, "connect_sensor_check_esp"):
            self.host.connect_sensor_check_esp(port)
        if hasattr(self.host, "refresh_sensor_check_info"):
            QTimer.singleShot(1200, lambda: self.host.refresh_sensor_check_info(notify=True))
        self.append_output("FL/GPIO13テストを書き込みました。センサチェックで受信値を更新します。")
        QMessageBox.information(
            self,
            "書き込み完了",
            "FL/GPIO13テストを書き込みました。\nセンサチェックでESP接続と更新を確認してください。",
        )

    def cleanup_serial_users(self) -> None:
        if self.worker is not None:
            self.append_output("処理中のため、シリアル停止は待機してください。")
            return
        cleanup = getattr(self.host, "stop_other_serial_communications_for_upload", None)
        if cleanup is None:
            self.append_output("シリアル停止処理が未実装です。")
            return
        messages = cleanup(None)
    ...
    def _start_worker(self, mode: str) -> None:
        if hasattr(self, "write_tabs"):
            self.write_tabs.setCurrentIndex(2)
        ...
        if isinstance(result, ArduinoCliResult):
            ...
            if mode == "compile_for_upload":
                ...
                if result.success:
                    ...
                    self.easy_status_label.setText(f"{done_text}シリアルへ自動移動します。")
                    self.append_output(f"{done_text}シリアルへ自動移動します。")
                else:
    ...
    def _update_safety_label(self) -> None:
        ...
        elif sketch == "custom_board_fl_gpio13_value_test":
            safety = "FL/GPIO13値読み取り専用 / Wi-Fi・Bluetooth・モータ出力なし"
