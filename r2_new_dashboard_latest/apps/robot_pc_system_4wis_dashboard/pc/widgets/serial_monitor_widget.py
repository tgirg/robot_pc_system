...
class SerialMonitorWidget(QWidget):
    def __init__(self, host) -> None:
        ...
        self.lsb_status_label = QLabel("LSB基板: 未確認")
        self.lsb_status_label.setObjectName("diagnosticLabel")
    ...
    def _build(self) -> None:
        ...
        layout.addWidget(self.lsb_status_label)
    ...
    def refresh_ports(self, log: bool = True) -> None:
        ...
        if not ports:
            fallback = current or self._host_serial_port() or "COM10"
            self.port_combo.addItem(fallback, fallback)
        if not ports and self.port_combo.findData("COM10") < 0:
    ...
    def connect_monitor(self) -> None:
        ...
        try:
            ...
            lsb_widget = getattr(self.host, "lsb_sensor_board_widget", None)
            if lsb_widget is not None and hasattr(lsb_widget, "set_connection_problem"):
                lsb_widget.set_connection_problem(str(exc))
            return
    ...
    def prepare_for_immediate_start(self, auto_connect: bool = True) -> None:
        ...
        if status.connected and not status.mock and self._host_serial_port().upper() == str(port).upper():
            if auto_connect and self._disconnect_host_serial_for_monitor(port):
                self.append_log("シリアルモニタ開始のため実機接続を切断しました")
            else:
    ...
    def _handle_normal_connection_conflict(self, port: str) -> bool:
        status = self._host_serial_status()
        if not (status.connected and not status.mock and self._host_serial_port().upper() == str(port).upper()):
            return True
    ...
    def _host_serial_status(self):
        serial_obj = getattr(self.host, "serial", None)
        if serial_obj is not None and hasattr(serial_obj, "status"):
            return serial_obj.status()
    ...
    def _disconnect_host_serial_for_monitor(self, port: str) -> bool:
        serial_obj = getattr(self.host, "serial", None)
        if serial_obj is not None and hasattr(self.host, "disconnect_esp32_from_ui"):
        ...
        if hasattr(controller, "reconnect_active"):
            controller.reconnect_active = False
        if hasattr(controller, "reconnect_phase"):
            controller.reconnect_phase = "serial_monitor"
        return True
    ...
    def _update_serial_status_summary(self) -> None:
        ...
        lsb_id_match = re.findall(r"^LSB,ID,([^,\r\n]+),?([^,\r\n]*)", text, flags=re.MULTILINE)
        lsb_sens_match = re.findall(r"^LSB,SENS,([^,\r\n]+)", text, flags=re.MULTILINE)
        lsb_err_match = re.findall(r"^LSB,ERR,([^,\r\n]+),?([^\r\n]*)", text, flags=re.MULTILINE)
        if lsb_err_match:
            code, detail = lsb_err_match[-1]
            suffix = f" / {detail}" if detail else ""
            self.lsb_status_label.setText(f"LSB基板: ERROR {code}{suffix}")
            self.lsb_status_label.setStyleSheet("color:#fca5a5; font-weight:700;")
        elif lsb_id_match or lsb_sens_match:
            board, version = lsb_id_match[-1] if lsb_id_match else ("LSB", "")
            seq = lsb_sens_match[-1] if lsb_sens_match else "-"
            version_text = f" / FW {version}" if version else ""
            self.lsb_status_label.setText(f"LSB基板: OK {board}{version_text} / seq {seq}")
            self.lsb_status_label.setStyleSheet("color:#86efac; font-weight:700;")
        else:
            self.lsb_status_label.setText("LSB基板: 未確認")
            self.lsb_status_label.setStyleSheet("color:#fef08a; font-weight:700;")
