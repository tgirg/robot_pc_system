...
class SensorPanel(QWidget):
    ...
    def update_values(
        ...
        self.values["LSB status"].setText(self._lsb_status_text(data))
        self.values["LSB ToF"].setText(
            f"前 {getattr(data, 'lsb_tof_front_mm', 0.0):.0f} / "
            f"右 {getattr(data, 'lsb_tof_right_mm', 0.0):.0f} / "
            f"後 {getattr(data, 'lsb_tof_rear_mm', 0.0):.0f} / "
            f"左 {getattr(data, 'lsb_tof_left_mm', 0.0):.0f} mm"
        )
        self.values["LSB ultrasonic"].setText(
            f"前L {getattr(data, 'lsb_us_front_l_mm', 0.0):.0f} / "
            f"前R {getattr(data, 'lsb_us_front_r_mm', 0.0):.0f} / "
            f"右F {getattr(data, 'lsb_us_right_f_mm', 0.0):.0f} / "
            f"右R {getattr(data, 'lsb_us_right_r_mm', 0.0):.0f} / "
            f"後R {getattr(data, 'lsb_us_rear_r_mm', 0.0):.0f} / "
            f"後L {getattr(data, 'lsb_us_rear_l_mm', 0.0):.0f} / "
            f"左R {getattr(data, 'lsb_us_left_r_mm', 0.0):.0f} / "
            f"左F {getattr(data, 'lsb_us_left_f_mm', 0.0):.0f} mm"
        )
    ...
    def _show_no_data(self, current_command: str) -> None:
        zero_rows = {
            ...
            "LSB status": "LSB: 未接続",
            "LSB ToF": "前 0 / 右 0 / 後 0 / 左 0 mm",
            "LSB ultrasonic": "前L 0 / 前R 0 / 右F 0 / 右R 0 / 後R 0 / 後L 0 / 左R 0 / 左F 0 mm",
    ...
    @staticmethod
    def _lsb_status_text(data: SensorData) -> str:
        status = status_label(getattr(data, "lsb_status", "未接続"))
        if status == "OK":
            board = getattr(data, "lsb_board_id", "LSB") or "LSB"
            version = getattr(data, "lsb_fw_version", "")
            seq = getattr(data, "lsb_seq", 0)
            suffix = f" / FW {version}" if version else ""
            return f"LSB: OK（{board}{suffix} / seq {seq}）"
        if status == "ERROR":
            detail = getattr(data, "lsb_error", "") or "詳細なし"
            return f"LSB: ERROR（{detail}）"
        return "LSB: 未接続"
