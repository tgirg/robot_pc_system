...
class ImuState:
    ...
    lsb_error: str = ""
    lsb_tof_front_mm: int = 0
    lsb_tof_right_mm: int = 0
    lsb_tof_rear_mm: int = 0
    lsb_tof_left_mm: int = 0
    lsb_us_front_l_mm: int = 0
    lsb_us_front_r_mm: int = 0
    lsb_us_right_f_mm: int = 0
    lsb_us_right_r_mm: int = 0
    lsb_us_rear_r_mm: int = 0
    lsb_us_rear_l_mm: int = 0
    lsb_us_left_r_mm: int = 0
    lsb_us_left_f_mm: int = 0
    status: str = ""
    lsb_status: str = NO_DATA
    ...
    last_lsb_update_time: float = 0.0

    def update_from_message(self, message: dict) -> None:
        message_type = message.get("type")
        now = time.monotonic()

        if message_type == "LSB_ID":
            self.lsb_board_id = str(message.get("board", "") or "LSB")
            self.lsb_fw_version = str(message.get("version", ""))
            self._set_lsb_status("OK", now)
        elif message_type == "LSB_PONG":
            self._set_lsb_status("OK", now)
        elif message_type == "LSB_STATUS":
            self._set_lsb_status(message.get("status"), now)
        elif message_type == "LSB_ERR":
            detail = str(message.get("detail", ""))
            self.lsb_error = str(message.get("code", "ERR")) + (f": {detail}" if detail else "")
            self._set_lsb_status("ERROR", now)
        elif message_type == "LSB_I2C":
            items = [str(item) for item in message.get("items", [])]
            self.lsb_i2c_summary = " / ".join(items) if items else "未受信"
            self._set_lsb_status("OK", now)
        elif message_type == "LSB_SENS":
            self._apply_lsb_sensor_snapshot(message, now)
    ...
    def has_recent_lsb(self, timeout_s: float = 2.0) -> bool:
        return normalize_status(self.lsb_status) != NO_DATA and self.last_lsb_update_time > 0 and (time.monotonic() - self.last_lsb_update_time) <= timeout_s
    ...
    def to_sensor_data(self) -> SensorData:
        ...
        lsb_status = self.lsb_status if recent_data and self.has_recent_lsb() else NO_DATA
        ...
        if is_sensor_active(imu_status) and self.imu_source not in {"", NO_DATA, "未接続"}:
            imu_source = self.imu_source
        lidar_source = source_label_for_status(lidar_status, "実データ", "ESP32ダミー出力")
        if is_sensor_active(lidar_status) and self.lidar_source not in {"", NO_DATA, "未接続"}:
        ...
        return SensorData(
            ...
            lsb_board_id=self.lsb_board_id if is_sensor_active(lsb_status) else "未接続",
            lsb_fw_version=self.lsb_fw_version if is_sensor_active(lsb_status) else "",
            lsb_seq=self.lsb_seq if is_sensor_active(lsb_status) else 0,
            lsb_i2c_summary=self.lsb_i2c_summary if is_sensor_active(lsb_status) else "未受信",
            lsb_error=self.lsb_error if normalize_status(lsb_status) == "ERROR" else "",
            lsb_tof_front_mm=sanitize_distance(self.lsb_tof_front_mm, lsb_status),
            lsb_tof_right_mm=sanitize_distance(self.lsb_tof_right_mm, lsb_status),
            lsb_tof_rear_mm=sanitize_distance(self.lsb_tof_rear_mm, lsb_status),
            lsb_tof_left_mm=sanitize_distance(self.lsb_tof_left_mm, lsb_status),
            lsb_us_front_l_mm=sanitize_distance(self.lsb_us_front_l_mm, lsb_status),
            lsb_us_front_r_mm=sanitize_distance(self.lsb_us_front_r_mm, lsb_status),
            lsb_us_right_f_mm=sanitize_distance(self.lsb_us_right_f_mm, lsb_status),
            lsb_us_right_r_mm=sanitize_distance(self.lsb_us_right_r_mm, lsb_status),
            lsb_us_rear_r_mm=sanitize_distance(self.lsb_us_rear_r_mm, lsb_status),
            lsb_us_rear_l_mm=sanitize_distance(self.lsb_us_rear_l_mm, lsb_status),
            lsb_us_left_r_mm=sanitize_distance(self.lsb_us_left_r_mm, lsb_status),
            lsb_us_left_f_mm=sanitize_distance(self.lsb_us_left_f_mm, lsb_status),
        )

    def _set_lsb_status(self, status: object, now: float) -> None:
        self.lsb_status = normalize_status(status)
        if not is_sensor_active(self.lsb_status):
            self._clear_lsb_values()
        self._touch(now)
        self.last_lsb_update_time = now

    def _apply_lsb_sensor_snapshot(self, message: dict, now: float) -> None:
        self._set_lsb_status("OK", now)
        self.lsb_error = ""
        self.lsb_seq = int(sanitize_number(message.get("seq"), min_value=0.0))

        tof = list(message.get("tof") or [0.0, 0.0, 0.0, 0.0])
        while len(tof) < 4:
            tof.append(0.0)
        self.lsb_tof_front_mm = int(sanitize_distance(tof[0], self.lsb_status))
        self.lsb_tof_right_mm = int(sanitize_distance(tof[1], self.lsb_status))
        self.lsb_tof_rear_mm = int(sanitize_distance(tof[2], self.lsb_status))
        self.lsb_tof_left_mm = int(sanitize_distance(tof[3], self.lsb_status))
        self.lidar_status = "OK"
        self.lidar_source = "LSB ToF"
        self.front_distance_mm = self.lsb_tof_front_mm
        self.right_distance_mm = self.lsb_tof_right_mm
        self.rear_distance_mm = self.lsb_tof_rear_mm
        self.left_distance_mm = self.lsb_tof_left_mm
        self.last_lidar_update_time = now

        ultrasonic = list(message.get("ultrasonic") or [0.0] * 8)
        while len(ultrasonic) < 8:
            ultrasonic.append(0.0)
        us_values = [int(sanitize_distance(value, self.lsb_status)) for value in ultrasonic[:8]]
        (
            self.lsb_us_front_l_mm,
            self.lsb_us_front_r_mm,
            self.lsb_us_right_f_mm,
            self.lsb_us_right_r_mm,
            self.lsb_us_rear_r_mm,
            self.lsb_us_rear_l_mm,
            self.lsb_us_left_r_mm,
            self.lsb_us_left_f_mm,
        ) = us_values
        self.distance_status = "OK"
        self.distance_sensor_mm = self.lsb_us_front_l_mm
        self.last_distance_update_time = now
        ...
        if imu is not None:
            ...
            while len(imu_values) < 3:
                imu_values.append(0.0)
            self._set_imu_status("OK", now)
            self.imu_source = "LSB IMU"
            self.yaw = sanitize_angle(imu_values[0], self.imu_status)
            self.pitch = sanitize_angle(imu_values[1], self.imu_status)
            self.roll = sanitize_angle(imu_values[2], self.imu_status)
            self.last_imu_update_time = now
    ...
    def sensor_age_text(self) -> str:
        ...
        items = [
            ("LSB", self.lsb_status, self.last_lsb_update_time),
    ...
    def _clear_lsb_values(self) -> None:
        self.lsb_seq = 0
        self.lsb_tof_front_mm = 0
        self.lsb_tof_right_mm = 0
        self.lsb_tof_rear_mm = 0
        self.lsb_tof_left_mm = 0
        self.lsb_us_front_l_mm = 0
        self.lsb_us_front_r_mm = 0
        self.lsb_us_right_f_mm = 0
        self.lsb_us_right_r_mm = 0
        self.lsb_us_rear_r_mm = 0
        self.lsb_us_rear_l_mm = 0
        self.lsb_us_left_r_mm = 0
        self.lsb_us_left_f_mm = 0
