...
class FieldRenderer:
    ...
    def draw_objects(self, painter: QPainter, show_labels: bool = True) -> None:
        for item in self.model.get_objects():
            ...
            angle = math.radians(float(item.orientation_deg))
            arrow_length = max(10.0, min(qrect.width(), qrect.height()) * 0.35)
            start = qrect.center()
            end = QPointF(
                start.x() + math.cos(angle) * arrow_length,
                start.y() + math.sin(angle) * arrow_length,
            )
            painter.setPen(QPen(QColor("#ff8a1f"), 2.5))
            ...
            if show_labels:
                ...
                face = item.bottom_face if item.bottom_face != "NOT_CONFIGURED" else "下面未設定"
                painter.drawText(qrect, Qt.AlignmentFlag.AlignCenter, f"{item.label_ja}\n{item.orientation_deg:+.0f}° / {face}")
